...
svg.selectAll(".input-node")
  ...
  .on("mouseover", function(d, i) {
    d3.select(this)
      .attr("fill", "#FFA3FD")
      .attr("r", radius * 1.5);
    // Add a label to display the ID on screen
    svg.append("text")
      .attr("id", "label-" + this.id) // Use the circle's ID to create the text's ID
      .attr("x", d.x-40 )
      .attr("y", d.y-40 )
      // console.log("I : " + this.id);
      $(".Node-ID").text("I-" + this.id);
  })
  .on("mouseout", function(d, i) {
    d3.select(this)
      ...
      .attr("r", radius);
      $(".Node-ID").text("");
    // Remove the label when the mouse moves away from the node
   // svg.select("#label-" + this.id).remove(); // Use the circle's ID to select and remove the text element
  });
...
connections.on("mouseover", function() {
  ...
  connections.style("opacity", function() {
    return (this === currentConnection.node()) ? 1 : 0.5;

  })
  .style("stroke-width", function() {
    return (this === currentConnection.node()) ? "4" : "1";   
  })
  .style("stroke", function() {
    return (this === currentConnection.node()) ? "#233": "#999";   
